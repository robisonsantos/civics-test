<script lang="ts">
    ...
    const standardQuestions =
        module?.questions.filter((q) => q.type === "standard") ?? [];
    ...
    const passed = $derived(
        score / testQuestions.length >= CONFIG.MIN_PASS_RATE,
    );
</script>
    ...
    {:else}
        <div class="w-full max-w-2xl">
            {#if !isFinished}
                <!-- Test Interface -->
                <div
                    class="bg-white border border-slate-200 p-8 rounded-3xl shadow-xl shadow-slate-200/50"
                >
                    <div class="flex justify-between items-center mb-6">
                        <span
                            class="text-xs font-bold uppercase tracking-wider text-slate-400"
                            >Module Exam: {module.title}</span
                        >
                    ...
                    <h2
                        class="text-2xl font-serif font-bold text-slate-800 text-center mb-6 min-h-[80px]"
                    >
                    ...
                    {:else}
                        <div
                            class="mb-6 p-6 rounded-2xl bg-emerald-50 border border-emerald-100 animate-in fade-in slide-in-from-top-2 duration-300"
                        >
                            <p
                                class="text-sm font-medium text-emerald-700 mb-2"
                            >
                                Correct Answer(s)
                            </p>
                            <div class="text-left space-y-2">
                                {#each testQuestions[currentIdx]?.answers as answer}
                                    <div
                                        class="flex items-center gap-2 text-base leading-relaxed text-emerald-900"
                                    >
                                        <span
            ...
            {:else}
                <!-- Results Screen -->
                <div
                    class="bg-white border border-slate-200 p-12 rounded-3xl shadow-xl text-center"
                >
                    {#if passed}
                        <div
                            class="mb-6 inline-flex items-center justify-center w-20 h-20 rounded-full bg-emerald-100 text-emerald-600"
                        >
                            <Trophy size={40} />
                        </div>
                        <h1
                            class="text-3xl font-serif font-bold text-slate-800 mb-2"
                        >
                            Module Passed!
                        </h1>
                        <p class="text-slate-500 mb-8">
                            You scored {score} out of {testQuestions.length} correctly.
                        </p>
                        <a
                            href="/dashboard"
                            class="px-8 py-3 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-700 transition-colors block w-full sm:w-auto mx-auto"
                        >
                            Return to Dashboard
                        </a>
                    {:else}
                        <div
                            class="mb-6 inline-flex items-center justify-center w-20 h-20 rounded-full bg-red-100 text-red-600"
                        >
                            <CircleAlert size={40} />
                        </div>
                        <h1
                        ...
                        {#if missedQuestions.length > 0}
                            <div
                                class="mb-8 text-left p-6 bg-slate-50 rounded-2xl border border-slate-200"
                            >
                                <h3
                                    class="text-sm font-bold text-slate-700 uppercase tracking-wider mb-3"
                                >
                                    Review these items:
                                </h3>
                                <ul class="space-y-2">
                        ...
                        >
                            ...
                            <a
                                href="/study/{moduleId}"
                                class="px-8 py-3 bg-white border border-slate-200 text-slate-600 rounded-2xl font-bold hover:bg-slate-50 transition-colors"
                            >
                                Study Again
                            </a>
                        </div>
